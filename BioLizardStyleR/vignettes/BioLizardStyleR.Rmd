...
output:
  html_document:
    toc: true
    toc_float: true
    css: !expr system.file('css', 'biolizardstyle_rmd.css', package='BioLizardStyleR')
vignette: >
...
::: {#logo align="top"}
```{r echo = FALSE}
knitr::include_graphics(system.file("logo", "BiolizardLogo.png", package="BioLizardStyleR"))
```
:::
...
The CSS style file in this package allows to BioLizard-ify a Rmd knit to an HTML. Use the output section in the YAML header of your Rmd to add the css file and the table of contents (toc):
...
output:
  html_document:
    toc: true
    toc_float: true
    css: !expr system.file('css', 'biolizardstyle.css', package='BioLizardStyleR')
```

To add the BioLizard logo to the document, add the following code chunk into your Rmd file:

<pre><code>
::: {#logo align="top"}
&grave;&grave;&grave;{r echo = FALSE}
knitr::include_graphics(system.file("logo", "BiolizardLogo.png", package="BioLizardStyleR"))
&grave;&grave;&grave;
:::
</code></pre>
...
<!-- The three base colors can also be used directly as `blz_green`, `blz_blue` and `blz_yellow`. -->
...
treemap::treemap(data, index=c("type_1", "type_2"), vSize="n", type="index",
                 ...
                 fontfamily.title = "Red Hat Display", fontfamily.labels = "Red Hat Display", fontfamily.legend = "Red Hat Display",  #set the Red Hat Display font
                  fontsize.title = 22,  fontsize.labels = 18, fontsize.legend = 18)
```
