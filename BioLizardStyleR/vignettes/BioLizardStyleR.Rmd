...
# 1. Installation
To install the `BioLizardStyleR` package directly from GitHub, you will need to utilize the `devtools` package. If the installation 
fails due to an error with the fonts, please follow step 2.
...
## greens scale
testplot_sequential <- testplot + scale_color_biolizard(type = "discrete", scheme = "greens") + lizard_style()
testplot_sequential
...
ggplot(datapoly, aes(x = x, y = y)) +
  ...
  scale_fill_biolizard(type = "discrete", scheme = "greens") +
  lizard_style()
...
g <- g + lizard_style() +
      scale_fill_biolizard(type='continuous',scheme='greens')
g
...
fig <- plot_ly(
  ...
  y = ~Dim2,
  color = ~Group,
  colors = biolizard_pal_qualitative(2)) %>% 
  lizard_layout() %>% 
  layout(xaxis = list(title = paste0("PC1: ", round(mds$var.explained[1]*100), "% variance")),
         yaxis = list(title = paste0("PC2: ", round(mds$var.explained[2]*100), "% variance"))
  )
...
p <- plot_ly(
  ...
  y = ~-log10(pvalue),
  color = ~padj < 0.05,
  colors = biolizard_pal_qualitative(2, reverse = TRUE)) %>% 
  lizard_layout() %>% 
  layout(legend = list(title = list(text = "padj < 0.05")))
p
```
