...
.onLoad <- function(libname, pkgname) {
    ...
    } else if ("Red Hat Display" %in% systemfonts::system_fonts()$family) {
        assign("font_ready", TRUE, envir = .blz_env)
    ...
    if (is_ready) {
        if (method == "system_copy") {
            packageStartupMessage(
                "Red Hat Display found and synced to system fonts. Ready for BLZ style!"
            )
        } else {
            packageStartupMessage(
        ...
        packageStartupMessage(
            "WARNING: Red Hat Display font installation failed."
        )
        packageStartupMessage(
            "The package relies on this font for correct visualization."
        )
